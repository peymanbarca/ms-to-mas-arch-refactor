...
delta_c_reverse = [1.454, 1.603, 1.801, 2.339, 2.103, 2.135, 2.091, 1.952, 2.123]
...
print(f'c_ranked_rev_improve = { (np.sum(delta_c_reverse) -  np.sum(delta_c_ranked)) / np.sum(delta_c_reverse)}')
print(f'c_ranked_rand_improve = { (np.sum(delta_c_random) -  np.sum(delta_c_ranked)) / np.sum(delta_c_random)}')